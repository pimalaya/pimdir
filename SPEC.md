...
The schema version is held in SQLite's built-in `PRAGMA user_version`, and mirrored in `store_meta.version`. Version 1 is defined by [migrations/0001_init.sql](./migrations/0001_init.sql). The two MUST agree; an implementation opening a store where they disagree MUST treat it as corrupt.
