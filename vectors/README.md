...
Two cases pin the fallback keys: event-no-uid.ics the **`hash:`** key Annex A.2 derives for content with no usable `UID`, the FNV-1a 64 digest of the bytes, and mail-no-message-id.eml the **`alt:`** key Annex A.1 derives for a message with no `Message-ID`; checks/vectors.py re-derives both. Three of summaries.json's cases pin the **minted** `link_id` (STORAGE.md §9): a hint the collection already holds under another handle is filed under `dup:`, the hint, `#` and the handle, concatenated verbatim in that order, and sync vector 31 pins the same over a fallback. Each reuses the fixture of the case above it, since minting depends on the hint and the handle and never on the body, and each carries the `hint` and `handle` its key was built from, so a consumer rebuilds the key rather than pattern-matching the string.

A provisional handle in a sync case is `U+0001` followed by the link id (SYNC.md §2), written with its control character escaped; checks/vectors.py checks that every baseless binding carries one and no based binding does.
