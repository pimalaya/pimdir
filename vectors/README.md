...
- Every value was then reproduced through a second, independent path (coreutils `sha256sum` and `base32`), which is also what this repository's CI re-runs on every push. Every UTC normalisation in meta.json was reproduced against the tzdb with coreutils `date`, both daylight-saving transitions included.
